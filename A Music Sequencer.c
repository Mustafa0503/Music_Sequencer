/*
  ...
  ** to learn or understand that code.
  ...
  - Like A1, we will provide a test driver for you to test your
	...
	//////////////////////////////////////////////////////SSss????gcc -Wall -o what A2_test_driver.c -lm && what
*/
...
/*double helperReverse2(BST_Node *root)
{
	BST_Node *tra = root;
	BST_Node *pos = root;
	double Max_bar;
...
void helperReverse(BST_Node *root)
{
	BST_Node *temp = NULL;
	...
	root->right = temp;
	helperReverse(root->left);
	helperReverse(root->right);
}

BST_Node *helperReverse3(BST_Node *root, double sum)
{
	//helperReverse(root);
	if(root!=NULL)
	{
		...
		//printf("%d\n", sum);
		root->bar = sum - root->bar;
		root->index = 1 - root->index;
		helperReverse3(root->left, sum);
		helperReverse3(root->right, sum);
	}
	return root;
 ...
 * ******************************************************************************************/
int harmonizeIndex( double freq)
{
	for(int i = 0; i <100; i++)
	{
		if(freq == note_freq[i])
...
void harmonizeFilter(BST_Node *root, BST_Node *new)
{
	
	if(new->index>=0&&new->index<1)
	{
		BST_insert(root, new);
	}
	
}
void harmonizeHelperTra(BST_Node *root, int semitones, double time_shift)
{
	if(root==NULL)
	{
		return;
	}
	harmonizeHelperTra(root->left, semitones, time_shift);
	harmonizeHelperTra(root->right, semitones, time_shift);
	double index=0;
	double freqq=0;
	int i=0;
	int bar=0;
	...
	i = harmonizeIndex( root->freq);
	if(i+semitones<=99 && i+semitones>=0)
	{
		freqq = note_freq[i + semitones];

	
	//printf("%f\n", freq);
		new = newBST_Node(freqq, bar, index);
	// new->bar = bar;
	// new->freq = freqq;
	// new->index = index;
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	harmonizeHelperTra(root, semitones, time_shift);
